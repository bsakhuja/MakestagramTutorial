...
struct LikeService {
    ...
    static func create(for post: Post, success: @escaping (Bool) -> Void) {
        ...
        // We create a reference to the current user's UID. We'll use this to build the location of where we'll store the data for liking the post.
        let currentUID = User.current.uid
        
        let likesRef = Database.database().reference().child("postLikes").child(key).child(currentUID)
        
        // success callback returns a Bool on whether the network request was successfully executed and our like data was saved to the database.
